...
from orchestrator import TABRAIZ_PRE_STANDARD_PROMPT
...
def get_styled_system_prompt(style_name: str = None, persona: str = "Tehzeeb", user_profile: dict = None) -> str:
    ...
    if user_profile and isinstance(user_profile, dict) and user_profile.get("name"):
        ...
        if not is_male:
            ...
            base += f"\n\n{TABRAIZ_PRE_STANDARD_PROMPT}\n\n[عام گفتگو]: صارف کے سوال کا براہِ راست، شائستہ، جامع، مناسب اور خلاصے کی صورت میں (2 سے 3 جملے) پاکستانی اردو میں جواب دیں۔ کسی بھی قسم کی غیر ضروری ذاتی معلومات پوچھنے کا اصرار نہ کریں۔"
